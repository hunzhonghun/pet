...
#define GET_STREAM_ERRNOR_NUM  (0xa017800e)
#define USLEEP_20000 (20000)
...
static void *SAMPLE_COMM_AUDIO_AencProc(void *parg)
{
    ...
    while (pstAencCtl->bStart) {
		...
		if (s32Ret == GET_STREAM_ERRNOR_NUM) {
			usleep(USLEEP_20000);
			continue;
		}
		...
		// send stream to decoder and play for testing
		if (pstAencCtl->bSendAdChn == HI_TRUE) {
			s32Ret = HI_MPI_ADEC_SendStream(pstAencCtl->AeChn, &stStream, HI_TRUE);
			if (s32Ret != HI_SUCCESS) {
                printf("%s: SendStream(%d) failed with %#x!\n",  \
